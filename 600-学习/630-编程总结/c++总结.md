...
把“用户名+冒号+密码”用base64编码后的字符串放在http请求重的header authorization中发给服务器，这种方式叫http基本认证。

> 这种方式虽然用肉眼看不出来，但用应用程序很容易解码，所以不能用http在网络上传输，一定要用https传输，因为Https是加密的。

## 重放攻击的解决方案

方案1：在http请求中加上时间戳和数字签名，

- 签名是为了确保请求的有效性，因为签名是经过加密的，只有客户端和服务器端知道加密方式和key，第三方模拟不了，我们通过对sign进行验证来判断请求的有效性，如果sign验证失败则判定为无效的请求，反之有效，但数字签名不能阻止重放攻击，因为黑客可以抓取你的stamp和sign，不做任何的修改，然后发送请求，
- 时间戳是为了确保请求的时效性，我们将上一次请求的时间戳进行存储，在下一次请求时，将两次时间戳进行比对，如果此次请求的时间戳和上次的相同或者小于上次的时间戳，则判定此次请求为过时请求，无效。因为正常情况下，第二个请求的时间肯定比上一次的时间大。

## 弱网的场景

- 网络慢或延迟，导致加载时间长
- 网络中断，web服务器返回500等状态码
- 网络超时，http请求发出后，很久没有响应。
...
- 开源工具有：jmeter（性能测试的神器，一个http请求响应时间应该控制在200ms以内），postman
- 商业工具有：loadrunner，soapUI

>>>>>>> df1cc6d2696a65ce626f9eba2d6571d3f54391af
